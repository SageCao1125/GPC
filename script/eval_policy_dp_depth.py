...
class DP:
    def __init__(self, task_name, head_camera_type: str, checkpoint_num: int, expert_data_num: int, seed: int):
        self.policy = get_policy(f'checkpoints/{task_name}_{head_camera_type}_{expert_data_num}_{seed}_depth_only/{checkpoint_num}.ckpt', None, 'cuda:0')
...
def test_policy(task_name, Demo_class, args, dp: DP, st_seed, test_num=20):
    ...
    while succ_seed < test_num:
        ...
        Demo_class.setup_demo(now_ep_num=now_id, seed = now_seed, is_test = True, ** args)
        Demo_class.apply_dp_depth(dp, args)
...
def main(usr_args):
    ...
    save_dir = Path(f'eval_result/dp_depth/{task_name}_{usr_args.head_camera_type}/{usr_args.expert_data_num}')
