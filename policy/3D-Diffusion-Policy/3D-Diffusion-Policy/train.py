...
class TrainDP3Workspace:
            ...
            if ((self.epoch + 1) % cfg.training.checkpoint_every) == 0 and cfg.checkpoint.save_ckpt:
                ...
                if not cfg.policy.use_pc_color:
                    if cfg.policy.num_inference_steps == 100:
                        print("Saving DP3 with DDPM Scheduler")
                        if not os.path.exists(f'../checkpoints/{self.cfg.task.name}_{cfg.training.seed}_DDPM'):
                            os.makedirs(f'../checkpoints/{self.cfg.task.name}_{cfg.training.seed}_DDPM')
                        save_path = f'../checkpoints/{self.cfg.task.name}_{cfg.training.seed}_DDPM/{self.epoch + 1}.ckpt'
                    else:
                        print("Saving DP3 with DDIM Scheduler")
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000):
        ...
        if not cfg.policy.use_pc_color:
            if cfg.policy.num_inference_steps == 100:
                cprint("Loading DP3 with DDPM Scheduler", 'magenta')
                ckpt_file = pathlib.Path(f'./policy/3D-Diffusion-Policy/checkpoints/{cfg.task.name}_{cfg.head_camera_type}_{cfg.expert_data_num}_{cfg.training.seed}_DDPM/{checkpoint_num}.ckpt')
            else:
                print("Loading DP3 with DDIM Scheduler", 'magenta')
